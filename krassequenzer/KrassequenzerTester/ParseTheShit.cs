...
namespace KrassequenzerTester
{
	...
	{
		/// <summary>
		/// Parses the shit and writes C# code for an enum.
		/// </summary>
		public static void Run()
		{
			var theShit = @"1.	Acoustic Grand Piano	65.	Soprano Sax
...
64.	SynthBrass 2	128.	Gunshot";
			...
			var listOfTheShit = new List<Tuple<int, string>>();
			var match = Regex.Match(theShit, @"^(?<LeNo>[0-9]+)\.\t(?<LeNa>.+?)\t(?<RiNo>[0-9]+)\.\t(?<RiNa>.+?)\s*$", RegexOptions.Multiline);
			while (match.Success)
			{
				var leNo = Int32.Parse(match.Groups["LeNo"].Value);
				var leNa = match.Groups["LeNa"].Value;
				var riNo = Int32.Parse(match.Groups["RiNo"].Value);
				var riNa = match.Groups["RiNa"].Value;
				var le = new Tuple<int, string>(leNo, leNa);
				var ri = new Tuple<int, string>(riNo, riNa);
				listOfTheShit.Add(le);
				listOfTheShit.Add(ri);
				match = match.NextMatch();
			}

			foreach (var shit in listOfTheShit.OrderBy(x => x.Item1))
			{
				Debug.WriteLine("/// <summary>");
				Debug.WriteLine("/// " + shit.Item2);
				Debug.WriteLine("/// </summary>");
				
				var compileSafeName = Regex.Replace(shit.Item2.Replace(' ', '_'), @"[^a-zA-Z0-9_]", "");
				var enumValue = shit.Item1 - 1;
				Debug.WriteLine(compileSafeName + " = " + enumValue.ToString() + ", ");
			}
		}
	}
}
